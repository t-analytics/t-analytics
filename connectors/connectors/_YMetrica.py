...
class YandexMetrica:
    def __init__(self, access_token, client_name, view_id):
        ...
        self.report_dict = {
            "BASE": {
                "metrics": {
                    "ym_s_newUsers": "INTEGER", "ym_s_pageviews": "INTEGER", "ym_s_users": "INTEGER",
                    "ym_s_visits": "INTEGER"},
                "dimensions": {
...
access_token = "AgAAAAAStrHFAAMfZWsDRCNfFUfwvkaC7sT2F1c"
client_name = "LENTA"
view_id = 37945865
ya = YandexMetrica(access_token, client_name, view_id)

metrics = [re.sub('[_]', ':', param) for param in list(ya.report_dict['BASE']['metrics'].keys())]
dimensions = [re.sub('[_]', ':', param) for param in list(ya.report_dict['BASE']['dimensions'].keys())]
names, result_ya = ya.get_report("2020-05-01", "2020-05-01", metrics, dimensions)
df = pd.DataFrame(result_ya, columns=names)
df.to_csv("metrica.csv")
